...
void SX1272Init( RadioEvents_t *events )
{
	...
	TxTimeoutTimer.debugRef = 1;
	RxTimeoutTimer.debugRef = 2;
	RxTimeoutSyncWord.debugRef = 3;
...
{
    // Set RESET pin to 1
    GpioInit( &SX1272.Reset, LORA_RESET, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );

    // Wait 1 ms
    DelayMs( 1 );

    // Configure RESET as input
    GpioInit( &SX1272.Reset, LORA_RESET, PIN_INPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
            ...
            case MODEM_LORA:
                if( SX1272.Settings.LoRa.FreqHopOn == true )
                {
                    ...
                    if( ( RadioEvents != NULL ) && ( RadioEvents->FhssChangeChannel != NULL ) )
                    {
						...
						//AJOUT JF DEBUG
						//NRF_LOG_INFO( "FhssChangeChannel" );
                    }
                }
                break;
            default:
